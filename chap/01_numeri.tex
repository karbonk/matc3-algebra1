...
\end{empheq}
Per eseguire la moltiplicazione~$4\cdot 2$, che possiamo leggere ``quattro volte due'', dobbiamo addizionare 4 volte 2, cioè~$2+2+2+2$, e otteniamo~8.
...
\subsection{Sottrazione di numeri naturali}
...
Si può osservare allora che in~$\insN$ la sottrazione~$a-b$ è possibile solo se~$b$ è più piccolo o al più uguale ad~$a$.

\subsection{Divisione di numeri naturali}
...
\begin{exrig}
 ...
 \begin{esempio} Divisori e multipli.
  \begin{itemize}
  \item $12:3=4$ perché~$3\cdot 4=12$. Quindi,~12 è divisibile per~3;~3 è un divisore di~12;~12 è un multiplo di~3; 
  \item 20 è divisibile per~4 perché~$20:4=5$;
  \item 7 è divisore di~35 perché~$35:7=5$;
  \item 6 è multiplo di~3 perché~$6=2\cdot 3$;
  \item 5 non è multiplo di~3 poiché non esiste alcun numero naturale che moltiplicato per~3 dà~5.
  \end{itemize}
 \end{esempio}
\end{exrig}
...
\begin{exrig}
 \begin{esempio} Divisione con resto.
\begin{multicols}{3}
  \begin{itemize}
  \item $0:2 =~0$;
  \item $1:2 =~0$ con resto~1;
  \item $5:2 =~2$ con resto~1.
  \end{itemize}
\end{multicols}
 \end{esempio}
\end{exrig}
...
\begin{exrig}
 \begin{esempio}
 Dovendo raggruppare 321~studenti in classi da 30~alunni, dividiamo~$312:20=10$ con resto~12.
 I rimanenti 12~alunni possono formare un'altra classe oppure possono essere distribuiti nelle altre classi.
  \end{esempio}
\end{exrig}
...
\item $16 \divint 9=1$;
\item $3\divint 5=0$;
\item $3\divint 0 = $ non si può fare.
\end{itemize*}
\end{multicols}
 \end{esempio}
\end{exrig}
...
Un'operazione ($\diamond$) ha un \emph{elemento neutro} $n$ se componendo~$n$ con qualsiasi altro numero lo lascia invariato, sia quando
...
\begin{exrig}
 ...
 L'operazione estrazione di radice
  \begin{itemize*}
    \item $\sqrt[5]{32} = 2$,~~infatti $2^5 = 2\cdot 2\cdot 2\cdot 2\cdot 2 = 32$.
    \item $\sqrt[3]{125} = 5$,~~infatti $5^3 = 5\cdot 5\cdot 5 = 125$.
    \item $\sqrt[4]{81} = 3$,~~infatti $3^4 = 3\cdot 3\cdot 3\cdot 3 = 81$.
    \item $\sqrt[7]{1} = 1$,~~infatti $1^7 = 1\cdot 1\cdot 1\cdot 1\cdot 1\cdot 1\cdot 1 = 1$.
  \end{itemize*}
 \end{esempio}
\end{exrig}
...
più grande tra tutti i divisori comuni ad~$a$ e~$b$ e viene indicato con~$\mcd(a\text{,~}b)$.
\end{definizione}
...
\begin{exrig}
 \begin{esempio} $\big\lbrace\big[3\cdot5-\big(5\cdot2-4\big)\big]\cdot2\big\rbrace:\big[\big(5\cdot6\big):\big(3\cdot5\big)+5:5\big]-\big\lbrace\big(5^2\cdot4\big):10-3^2\big\rbrace+1$.
 \begin{align*}
 \big\lbrace\big[3\cdot5-\big(5\cdot2-4\big)\big]\cdot2\big\rbrace&:\big[\big(5\cdot6\big):\big(3\cdot5\big)+5:5\big]-\big\lbrace\big(5^2\cdot4\big):10-3^2\big\rbrace+1\\
			 ...
			 &=\big\lbrace\big[3\cdot5-6\big]\cdot2\big\rbrace:\big[2+1\big]-\big\lbrace100:10-9\big\rbrace+1\\
